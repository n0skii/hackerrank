...
def bomberMan(n, grid):
    ...
    for i in range(1, 7):
        ...
        if i % 2 == 0:
            for y in range(len(grid)):
                newGrid[y] = [(3 if elem <= 0 else elem) for elem in newGrid[y]]
        else:
            for y in range(len(grid)):
            ...
            for i in range(len(newGrid)):
                newGrid[i] = [(0 if elem == -1 else elem) for elem in newGrid[i]]
    ...
    gridToReturn = [
        "".join(
            [
                ("." if uniqueGrids[index][i][y] == 0 else "O")
                for y in range(len(uniqueGrids[index][i]))
            ]
        )
        for i in range(len(uniqueGrids[index]))
    ]
